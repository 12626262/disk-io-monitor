...
Windows 本地监控小工具：持续采集**每天每个应用读写了多少数据**,并能实时看见当前磁盘的读写，并在浏览器里用图表可视化展示。
...
- **立即记录并打开仪表盘**：双击 `bin\立即记录并打开仪表盘.bat`，或点击仪表盘上的“立即记录并刷新”按钮，
  采集器立即把当前数据写入数据库并打开/刷新仪表盘。
- **实时文件读写监控**：仪表盘页面点击“开始实时监控”，通过 Windows ETW（内核文件事件）实时显示
  每个进程对每个文件的读写量；再次点击停止。首次使用会弹出 UAC 管理员授权。

> ETW 采集基于 FireEye pywintrace（Apache 2.0），源码内置在 `etw\` 目录。
